...
const CenterSeimaSales: React.FC<CenterSeimaSalesProps> = ({
  ...
  // Helper function to calculate net profit for a sale
  const calculateSaleNetProfit = (sale: SalesData) => {
    return sale.sold - sale.rent - sale.expenses - sale.exits;
  };
  ...
  const fetchSales = useCallback(async () => {
    ...
    try {
      ...
      console.log('...', `${API_BASE_URL}/api/center-seima-sales`); // Debug log

      const response = await fetch(`${API_BASE_URL}/api/center-seima-sales`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingSale 
        ? `${API_BASE_URL}/api/center-seima-sales/${editingSale._id}`
        : `${API_BASE_URL}/api/center-seima-sales`;
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = Cookies.get('accessToken');
      const response = await fetch(`${API_BASE_URL}/api/center-seima-sales/${deleteSaleId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const permissions = getRolePermissions('مبيعات سنتر سيما');
                  ...
                  <Card className="bg-gray-800/40 border-purple-500/20">
                    <CardHeader className="pb-2">
                      <CardTitle className="text-white text-right flex items-center justify-end text-xs sm:text-sm">
                        ...
                        <ArrowUpCircle className="w-4 h-4 sm:w-5 sm:h-5 text-purple-400" />
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      ...
                      {sales.map((sale) => (
                        ...
                        >
                          ...
                          <TableCell className="text-right">
                            <span className={`font-semibold ${calculateSaleNetProfit(sale) >= 0 ? 'text-purple-300' : 'text-red-400'}`}>
