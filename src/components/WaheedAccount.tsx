...
interface WaheedAccountProps {
  isOpen: boolean;
  onClose: () => void;
}

const WaheedAccount: React.FC<WaheedAccountProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  const [accounts, setAccounts] = useState<WaheedAccountData[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedDate, setSelectedDate] = useState<Date>();
  const [calendarOpen, setCalendarOpen] = useState(false);
  const [editingAccount, setEditingAccount] = useState<WaheedAccountData | null>(null);
  ...
  const [deleteAccount, setDeleteAccount] = useState<WaheedAccountData | null>(null);
  ...
  const fetchAccounts = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        'https://waheed-web.vercel.app/api/waheed-account',
        {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingAccount 
        ? `https://waheed-web.vercel.app/api/waheed-account/${editingAccount._id}`
        : 'https://waheed-web.vercel.app/api/waheed-account';
  ...
  const handleEdit = (account: WaheedAccountData) => {
  ...
  const handleDelete = (account: WaheedAccountData) => {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const response = await fetch(
        `https://waheed-web.vercel.app/api/waheed-account/${deleteAccount._id}`,
        {
          method: 'DELETE',
          headers: getAuthHeaders(),
        }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-black border-gray-700/50 p-0">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          />
          <motion.div
            className="absolute -bottom-20 -left-20 w-32 h-32 bg-red-500/10 rounded-full blur-3xl"
            animate={{
              scale: [1.2, 1, 1.2],
              rotate: [360, 180, 0],
            }}
            ...
            <Card className="bg-gray-800/40 border-gray-700/30 mb-6">
              <CardHeader>
                ...
                <CardDescription className="text-gray-400 text-right">
                  {editingAccount ? 'تعديل بيانات حساب وحيد سعيد' : 'أدخل بيانات حساب وحيد سعيد'}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form
                ...
                >
                  ...
                  {/* Submit Button */}
                  <motion.div
                    className="md:col-span-2 lg:col-span-3 flex justify-end gap-3"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    {editingAccount && (
                      <Button
                        ...
                        variant="outline"
                        className="bg-gray-700 border-gray-600 text-gray-300 hover:bg-gray-600 px-6 py-3 rounded-xl font-medium transition-all duration-200"
                      >
                        إلغاء
                      </Button>
                    )}
                    <Button
                      ...
                      className="bg-gradient-to-r from-orange-600 to-red-700 hover:from-orange-700 hover:to-red-800 text-white px-8 py-3 rounded-xl font-medium transition-all duration-200 shadow-lg hover:shadow-orange-500/25"
                    >
                      {isSubmitting ? (
                        <>
            ...
            <CardHeader className="border-b border-gray-700/30">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3 space-x-reverse">
                  <motion.div
                    className="p-2 bg-gradient-to-r from-orange-600 to-red-700 rounded-lg"
                    whileHover={{ scale: 1.05 }}
                  >
              ...
              <CardDescription className="text-gray-400 text-right">
                جميع سجلات حسابات وحيد سعيد البلينا
              </CardDescription>
            </CardHeader>
        ...
        <AlertDialogContent className="bg-gray-900 border-gray-700 text-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-300 text-right">
              هل أنت متأكد من حذف سجل وحيد سعيد لتاريخ "{deleteAccount?.date}"؟
              <br />
